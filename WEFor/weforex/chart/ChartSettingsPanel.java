...
public class ChartSettingsPanel extends ChartView implements ActionListener,
	...
	public static final class ComboBoxRenderer extends JLabel implements ListCellRenderer {
	...
	private static final Dimension GRAPH_LIST_ELEMENT_DIMENSION = new Dimension(100, 20);
	private static final Dimension DRAWING_LIST_ELEMENT_DIMENSION = new Dimension(100, 20);
	 ...
	 * @param correspondingChartPanel the graph on which settings in this panel
	 *            will reflect
	 * @param correspondingDrawingPanel the drawing panel on which settings in
	 *            this panel will reflect
	 * @param initPeriod the initial period the settings panel should adjust to
	 * @param initOfferSide the initial offer side the settings panel should
	 *            adjust to
	 */
	ChartSettingsPanel(ChartController controller, GraphPanel correspondingGraphPanel,
		...
		graphList.setSelectedItem(correspondingGraphPanel.getGraph().getClass());
	...
	public void actionPerformed(final ActionEvent evt) {
		...
		if (evt.getSource() == instrumentList) {
			...
			controller.changeInstrument((Instrument) instrumentList.getSelectedItem());
			...
			if (controller.getPeriod().equals(Period.TICK)
				...
				controller.changePeriod(Period.TEN_SECS);
			...
			correspondingGraphPanel.getGraph().zoom(ForexConstants.ZOOM_IN_MAGNIFICATION);

		} else if (evt.getSource() == zoomOutBtn) {

			correspondingGraphPanel.getGraph()
					.zoom(ForexConstants.ZOOM_OUT_MAGNIFICATION);
