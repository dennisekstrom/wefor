...
public final class TimeRange implements Comparable<TimeRange> {
	 ...
	 * Create a TimeRange.
	 * 
	 * @param time1 one edge
	 * @param time2 the other edge
	 */
	 ...
	 * 
	 * @param range the TimeRange to check if it's overlapping
	 * @return true if the given TimeRange overlaps this TimeRange, otherwise
	 *         false
	 */
	public boolean overlaps(TimeRange range) {
		return range.startTime < this.endTime && range.endTime > this.startTime;
	}
	...
	public String toString() {
		return this.getClass().getName() + " start: "
				+ ForexTools.getTimeRepresentation(startTime) + " end: "
				+ ForexTools.getTimeRepresentation(endTime);
	}
}