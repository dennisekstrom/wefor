...
public class ChartPanel extends ChartView {
	 ...
	 * components.
	 */
	ChartPanel(ChartController controller, TimeRelativeFeed feed, boolean showSettings) {
		super(controller);
		...
		if (controller.getPeriod().equals(Period.TICK))
			graph = new SingleLineGraph(controller, timeAxis, rateAxis, feed);
		else
			graph = new CandleGraph(controller, timeAxis, rateAxis, feed);
